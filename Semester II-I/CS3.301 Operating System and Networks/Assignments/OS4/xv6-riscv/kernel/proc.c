...
found:
  ...
  if((p->trapframe_copy = (struct trapframe *)kalloc()) == 0){
      ...
      uint64 pinlock = 1;
      uint64 interested = 0;
      return pinlock*interested+interested;
  }
  ...
  for(;;){
    ...
    for (newProcess = proc; newProcess < &proc[NPROC]; newProcess++)
      {
        ...
        {
          if (firstProcess == 0 || newProcess->creation_time < firstProcess->creation_time)
            {
                if (firstProcess !=0)
    ...
    if(process){
      ...
      for (p = proc; p < &proc[NPROC]; p++)
      {
        ...
        int flag2 = (dp == processDp && p->numScheduled == process->numScheduled && p->creation_time < process->creation_time);
...
int turn_variable(void){
  uint32 val = 0;
  return val;
}

uint64 sys_sigalarm(void){
    uint64 turn = turn_variable();
    int ticks;
    argint(0, &ticks);
    uint64 handler;
    argaddr(1, &handler);
