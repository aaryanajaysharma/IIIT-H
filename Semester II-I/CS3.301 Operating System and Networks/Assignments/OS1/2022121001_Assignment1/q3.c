...
int main(int argc, char *argv[])
{
    ...
    off_t newFileDescriptor = open(argv[1], O_RDONLY);
    ...
    off_t oldFileDescriptor = open(argv[2], O_RDONLY);
    // Check if the file is opened successfully
    ...
    long long unsigned int totalFileSize = lseek(newFileDescriptor, 0, SEEK_END);
    ...
    while (readFileSize > 0)
    {
        ...
        {
            if (newFileChunk[i] != oldFileChunk[chunkSize - i - 1] && lseek(newFileDescriptor, 0, SEEK_CUR) != totalFileSize)
            {
        ...
        if(flag == 0)
        {
            off_t index = lseek(newFileDescriptor, 0, SEEK_CUR);
            char x;
            read(newFileDescriptor, &x, 1);
            printf("%lld\n", index);
            printf("%c\n", x);
            printf("%llu\n", totalFileSize);
            break;
